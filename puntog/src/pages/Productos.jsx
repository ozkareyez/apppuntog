...
const Productos = () => {
  ...
  // 🔥 FUNCIÓN PARA CREAR SLUG DINÁMICO
  const createSlug = (nombre) => {
    return nombre
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
  ...
  useEffect(() => {
    ...
    // 🔥 MODIFICADO: Buscar por ID de categoría usando el slug
    if (categoriaActual !== "todas") {
      // Encontrar la categoría que coincida con el slug
      const categoriaEncontrada = categorias.find(
        (cat) => createSlug(cat.nombre) === categoriaActual,
      );

      if (categoriaEncontrada) {
        url += `categoria_id=${categoriaEncontrada.id}&`;
      }
    }
  ...
  }, [categoriaActual, filtroOferta, categorias]); // 🔥 Añadido categorias a dependencias
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-3 mb-6 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          ${
            categoriaActual === "todas"
              ...
              {/* 🔥 MODIFICADO: Usar slug dinámico */}
              {categorias.map((cat) => {
                const slug = createSlug(cat.nombre);
                return (
                  <button
                    key={cat.id}
