...
const Productos = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="animate-spin rounded-full h-14 w-14 border-t-2 border-b-2 border-pink-500"></div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productos.map((producto) => {
            ...
            const esOferta =
              producto.es_oferta && precioAntes > precio && producto.descuento;

            return (
              <div
                key={producto.id}
                onClick={() => navigate(`/productos/${producto.id}`)}
                className="relative bg-[#1f1f1f] border border-white/10 rounded-2xl overflow-hidden cursor-pointer hover:border-pink-400 transition-all"
              >
                {/* 🏷️ ETIQUETA OFERTA */}
                {esOferta && (
                  <div className="absolute top-3 left-3 z-10 flex items-center gap-1 bg-gradient-to-r from-pink-500 to-pink-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg">
                    <Tag size={12} />
                    {producto.descuento}% OFF
                  </div>
                )}
                ...
                <div className="p-4 text-center">
                  ...
                  <button
                    ...
                    className={`w-full py-2 rounded-xl font-semibold flex justify-center gap-2 transition-all ${
                      esOferta
                        ? "bg-gradient-to-r from-pink-500 to-pink-600 text-white shadow-md hover:shadow-pink-500/40"
                        : "bg-white text-black hover:bg-pink-500 hover:text-white"
                    }`}
                  >
                    <ShoppingCart size={16} />
