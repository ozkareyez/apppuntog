...
app.get("/api/productos", (req, res) => {
  ...
  if (categoria && categoria !== "todas") {
    query += " AND p.categoria_id = (SELECT id FROM categorias WHERE slug = ?)";
...
app.post("/api/productos", (req, res) => {
  ...
  DB.query(
    ...
    (categoria, nombre, talla, color, precio, imagen, categoria_id,
     precio_antes, descuento, es_oferta, descripcion, activo)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,1)
    `,
    [
      categoria,
    ...
    (err, result) => {
      ...
      res.json({ ok: true, producto_id: result.insertId });
    }
  );
});
