  ...
  try {
    ...
    try {
      ...
      if (!tableExists) {
        ...
        console.log("💡 Debes crearla manualmente");
        return;
      }
      ...
      if (!hasUsers) {
        console.log("👥 Tabla vacía. Insertando usuarios iniciales...");
          ...
          try {
            await DB.promise().query(
              ...
              [
                ...
                userData.icon,
                userData.visible,
                userData.permissions || JSON.stringify([]),
              ],
            );
        ...
        console.log(`✅ Tabla ya tiene ${userCount[0].count} usuario(s)`);
      }
    } catch (error) {
      console.error("❌ Error verificando tabla:", error.message);
    }
  } catch (error) {
    console.error("❌ Error en initializeUsers:", error.message);
  }
};
